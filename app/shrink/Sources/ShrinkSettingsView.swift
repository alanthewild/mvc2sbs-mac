...
let shrinkHelpSections: [ShrinkHelpSection] = [
    ...
    ShrinkHelpSection(title: "Reading a plan", rows: [
        ...
        ShrinkHelpRow(heading: "PSNR", body: "Treat it as a weak signal. Across four films it did not predict what the eye saw, and the only real failure scored higher than a film that was fine. Low PSNR on grainy content just means the grain differs, which is invisible. PSNR is also nearly blind to banding, because the error is small and spread across a large flat area, which is exactly what an average hides."),
    ...
    ShrinkHelpSection(title: "What it cannot tell you", rows: [
        ...
        ShrinkHelpRow(heading: "Whether the original language survives", body: "The rules keep every whitelisted language plus the first audio track. That covers a foreign film only when the original language comes first. No metadata reliably identifies a film's original language, so this cannot be fixed in general. What the tool does instead is name the loss in the reason column."),
    ...
    ShrinkHelpSection(title: "Settings worth knowing", rows: [
        ...
        ShrinkHelpRow(heading: "Minimum size", body: "5 GB by default. Low bitrate web sources inflate rather than shrink, with ratios above 1.0, and this gate catches most of them before any probe is spent on them."),
