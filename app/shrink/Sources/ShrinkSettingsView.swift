...
let shrinkHelpSections: [ShrinkHelpSection] = [
    ...
    ShrinkHelpSection(title: "What it cannot tell you", rows: [
        ...
        ShrinkHelpRow(heading: "What has not been tested", body: "Both replace modes have now been run on real media, including in place with the film on a NAS and scratch on a local disk, which is the cross-filesystem case the copy-back safety exists for. Still untested: the strip-only path on an already-HEVC file, and anything carrying 3D side-by-side geometry. See docs/mkvshrink-status.md in the repository, which is deliberately honest about this."),
    ]),
